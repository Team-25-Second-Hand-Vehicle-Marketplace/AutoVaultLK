...
describe('InMemoryDictionarySnapshot', () => {
  ...
  describe('vocabulary', () => {
    ...
    const vocab = new InMemoryDictionarySnapshot([TOYOTA, HONDA, COROLLA, CIVIC]);

    it('lists every canonical make', () => {
      expect(vocab.vocabulary().makes.sort()).toEqual(['Honda', 'Toyota']);
    });

    it('groups models under their make', () => {
      const { modelsByMake } = vocab.vocabulary();
